...
// Cerca, función que dado dos valores, verifica que la distancia entre ambos sea menor a un épsilon definido
...
const byte MotorCtrl[3][2] = {{31, 33}, {35, 37}, {39, 41}}; // Cambiar A0 por 30 para uso en DUE
...
void RutinaEncoder(byte NEncoder) {
  ...
  if (Debounced(UltimoT[Num])) {
    ...
    if (Contador[Num] == Nverificador) {
      ...
      RapidezAngMed[Num] = double(Contador[Num]) / (Tiempo_Fin[Num] - Tiempo_Ini[Num]);
      RPMMed[Num] = RapidezAngMed[Num] * 60000.0 / TotalPerf;
